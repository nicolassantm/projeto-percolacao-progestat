...
def metricas_cluster(N, p, n):
    ...
    media_tam_percola = sum(tam_cluster_percola) / \
        total_percola if total_percola else 0
    media_tam_nao_percola = sum(
        tam_cluster_nao_percola) / total_nao_percola if total_nao_percola else 0

    media_num_percola = sum(num_cluster_percola) / \
        total_percola if total_percola else 0
    media_num_nao_percola = sum(
        num_cluster_nao_percola) / total_nao_percola if total_nao_percola else 0
...
def grafico_tam_clusters(N, p, n):
    ...
    plt.suptitle(
        f"Distribuição do tamanho dos maiores clusters (N={N}, p={p}, n={n})", fontsize=16)
    plt.tight_layout()
    plt.show()
...
def grafico_num_clusters(N, p, n):
    ...
    plt.suptitle(
        f"Distribuição do número de clusters (N={N}, p={p}, n={n})", fontsize=16)
    plt.tight_layout()
    plt.show()
...
def encontrar_limiar(N, n):
    ...
    def f(N, x, n):
        # retorna probabilidade de percolar
        return metricas_cluster(N, x, n)[2]
...
def limiar_critico(n):
    ''' Recebe como parâmetro n (número de amostras a serem geradas).
        Renorna listas contendo o limiar finito para cada valor de n e um gráfico com a estimativa do limiar crítico, quando N tende ao infinito'''
